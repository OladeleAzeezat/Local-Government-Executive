    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (SqlConnection conn = new SqlConnection(mainconn))
            {
                ...
                SqlCommand selectCmd = new SqlCommand("SELECT COUNT(*) FROM PuExcoTb where pos = @pos AND pu = @pu", conn);
                selectCmd.Parameters.AddWithValue("@pu", Request.Form[PuList.UniqueID].ToString());
                ...
                {
                    showAlert(PosList.SelectedItem.ToString() + " information for " + PuList.SelectedItem.ToString() + " exist");
                }
                else
                {
                    SqlCommand cmd = new SqlCommand("INSERT INTO PuExcoTb (lg, ward, pu, pos, name, num, card) VALUES (@lg, @ward, @pu, @pos, @name, @num, @card)", conn);
