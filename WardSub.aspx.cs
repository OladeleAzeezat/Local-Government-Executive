    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (SqlConnection conn = new SqlConnection(mainconn))
            {
                ...
                SqlCommand selectCmd = new SqlCommand("SELECT COUNT(*) FROM WardExcoTb WHERE lg = @lg AND pos = @pos AND ward = @ward", conn);
                selectCmd.Parameters.AddWithValue("@lg", txtLg.Text);
                selectCmd.Parameters.AddWithValue("@ward", WardList.SelectedItem.ToString());
                selectCmd.Parameters.AddWithValue("@pos", PosList.SelectedItem.ToString());
                int existingRecordsCount = (int)selectCmd.ExecuteScalar();

                if (existingRecordsCount > 0)
                {
                    
                    showAlert(PosList.SelectedItem.ToString() + " information for "  + WardList.SelectedItem.ToString() + " exist");
                }
                else
                {
                    SqlCommand cmd = new SqlCommand("INSERT INTO WardExcoTb (lg, ward, pos, name, num, card) VALUES (@lg, @ward, @pos, @name, @num, @card)", conn);
